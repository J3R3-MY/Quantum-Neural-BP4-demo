...
class NBP_oc(nn.Module):
    ...
    def BCE_loss(self, Gamma_p) -> torch.Tensor:
        """loss functions proposed in [1] eq. 11"""
        tmp = torch.clip(Gamma_p,min=-30)
        # first row, anti-commute with X, second row, anti-commute with Z, [1] eq. 10
        Gamma = torch.exp(-1*tmp)

        PI = 1/(1+torch.sum(Gamma,dim=1,keepdim=False))

        PX = PI * Gamma[:, 1, :]
        PZ = PI * Gamma[:, 2, :]
        PY = PI * Gamma[:, 0, :]

        assert not torch.isinf(PX).any()
        assert not torch.isinf(PX).any()
        assert not torch.isnan(PZ).any()
        assert not torch.isnan(PZ).any()
        assert not torch.isnan(PY).any()
        assert not torch.isnan(PY).any()
        ...
        loss = torch.zeros(1, self.batch_size)
        for b in range(self.batch_size):
            loss[0,b] = torch.matmul(PX[b],torch.transpose(self.qx[b], dim0=0, dim1=-1))+\
            torch.matmul(PZ[b], torch.transpose(self.qz[b], dim0=0, dim1=-1))+\
            torch.matmul(PY[b], torch.transpose(self.qy[b], dim0=0, dim1=-1))+\
            torch.matmul(PI[b],torch.transpose(self.qi[b], dim0=0, dim1=-1))
    ...
    def forward(self, errorx: torch.Tensor, errorz: torch.Tensor, ep: float, batch_size=1, degenerate_loss = True) -> torch.Tensor:
        ...
        for i in range(self.n_iterations):
            ...
            if degenerate_loss:
                ...
                loss_array[:, i] = self.BCE_loss(Tau)
    ...
    def ini_weight_as_one(self, n_iterations: int):
        ...
        if self.m_oc<self.n:
            ...
            self.ini_weights = np.array([1.0,0.15])
...
def optimization_step(decoder, ep0, optimizer: torch.optim.Optimizer, errorx, errorz, scheduler=None, degnerate_loss = True):
    # call the forward function

    loss, loss_min = decoder(errorx, errorz, ep0, batch_size=batch_size, degenerate_loss=degnerate_loss)
...
#             pbar.update(1)
#             pbar.set_description(f"loss {552.9938+ loss[idx]}")
#             idx += 1
#             if ((i_batch+1)%100==0):
#                 decoder.save_weights()
#                 plot_loss(loss[0:idx-1], path=None)
#
#     decoder.save_weights()
#     print('Training completed.\n')
#     return loss
def training_loop(decoder, optimizer, ep1, sep,num_points, ep0, num_batch, path, scheduler=None, degenerate_loss = True):
    ...
    with tqdm(total=loss_length) as pbar:
        for i_batch in range(num_batch):
            ...
            loss[idx], loss_min[idx] = optimization_step(decoder, ep0, optimizer, errorx, errorz,scheduler, degnerate_loss=degenerate_loss)
            pbar.update(1)
...
decoder = NBP_oc(n, k, m, m1, m2, codeType, n_iterations, batch_size=batch_size, folder_weights=path)
...
cpp_parameters = ['-d',str(n),str(k), str(m), '35', '1', '-i',str(ep0),'-r','0.15','0.015','0.045', '-e', '100']
# pre-training stage, basically only the parameters for the first iteration is trained
loss = torch.Tensor()

for _ in range(10):
    loss_pre_train = training_loop(decoder, optimizer, ep1, sep,num_points, ep0, n_batches, path, scheduler=scheduler, degenerate_loss=False)
    loss = torch.cat((loss, n+loss_pre_train), dim=0)
    plot_loss(loss, path)
    subprocess.call([cpp_executable] + cpp_parameters)

    # loss_pre_train = training_loop(decoder, optimizer, ep1+0.05, sep,num_points, ep0, n_batches, path, scheduler=scheduler, degenerate_loss=True)
    # loss = torch.cat((loss, loss_pre_train), dim=0)
    # plot_loss(loss, path)
    # subprocess.call([cpp_executable] + cpp_parameters)

# cpp_parameters2 = ['-d',str(n),str(k), str(m), '100', '1', '-i',str(ep0),'-r','0.15','0.015','0.03']
# subprocess.call([cpp_executable] + cpp_parameters2)
...
#
# n_batches = 1500
# loss = training_loop(decoder, optimizer, r1, r2, ep0, n_batches, path)
#
